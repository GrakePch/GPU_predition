...
// Kernel for testing setp.f32
__global__ void microbenchmark(float *input1, float *input2, float *output) {
    float temp = *input1;

    for (int i = 0; i < N; i++) {
        temp = (temp < *input2) ? 1.0f : 0.0f; // setp.f32 operation (conditional check)
    }
    *output = temp;
}
...
int main(int argc, char *argv[]) {
    ...
    int gpu_id = atoi(argv[2]);    // We prefer 1 because less people use it
    ...
    printf("setp.f32 delay: %f cycles\n", cycles);
